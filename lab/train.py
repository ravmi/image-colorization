            ...
            if epoch % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    for xs, ys, yps in zip(x, y, y_pred[:10]):
                        # xs - grayscale (in lab)
                        # y - ground_truth_colors 
                        # y_ pred <- guessed colors (2dims in lab)
                        # first dim is channels
                        img = make_image(xs, yps)
                        img_gt = make_image(xs, ys)
                        '''

                        img = torch.cat(((xs + 0.5) * 100, yps * 100), dim=0).cpu().detach().numpy()
                        img = np.moveaxis(img, 0, -1)
                        img = color.lab2rgb(img)
                        img = Image.fromarray(np.uint8(img*255))

                        img_gt = torch.cat(((xs + 0.5) * 100, ys * 100), dim=0).cpu().detach().numpy()
                        img_gt = np.moveaxis(img_gt, 0, -1)
                        img_gt = color.lab2rgb(img_gt)
                        ...
                        #run['generated_image'].log(T.ToPILImage()(img))
                        run['generated_image'].log(img)
                        run['original_image'].log(img_gt)
